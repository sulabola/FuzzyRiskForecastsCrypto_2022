...
names(data)<-c("Price.APPL","Price.AMZN","Price.FB","Price.GOOG","Price.VIX")

### Daily plot of price
# Need to set the directory to save the plots
#pdf("tsplot.pdf", height=6, width=10) #sacf of the original series
plot(data, xlab="Days", main = "Daily price history of APPL, AMZN, FB, GOOG, VIX ",col=c("black","Blue","red", "green","grey", "purple"))
#dev.off()

return.cc = diff(log(data))
names(return.cc)<-c("APPL","AMZN","FB","GOOG","VIX")
...
corr1=percent(c(acf1$APPL$acf[2],acf1$AMZN$acf[2],acf1$FB$acf[2],acf1$GOOG$acf[2],acf1$VIX$acf[2])) #Extracting acf values
...
corr2=percent(c(acf2$APPL$acf[2],acf2$AMZN$acf[2],acf2$FB$acf[2],acf2$GOOG$acf[2],acf2$VIX$acf[2])) #Extracting acf values

# absolute of 
acf3=apply(abs(y1),2,acf,lag=1,plot=F)
corr3=percent(c(acf3$APPL$acf[2],acf3$AMZN$acf[2],acf3$FB$acf[2],acf3$GOOG$acf[2],acf3$VIX$acf[2])) #Extracting acf values
...
# }
# MAsummary=data.frame(names(ret.cc_rate),VaR_MA)
# MAsummary
# # ES_MA
...
```{r}
# SummaryCompare=data.frame(names(ret.cc_rate),HSsummary$VaRrate_HS,MAsummary$VaR_MA,EWMADanielsson$VaR_rateFixed,NGarchSummary$VaR_rateG,tGarchSummary$VaR_ratetG1,tGarchSummary$dfG)
# names(SummaryCompare)=c("Stock","HS","MA","EWMA","NGarch","tGarch","df")
# SummaryCompare
...
SummaryVar=data.frame(names(ret.cc_rate),HSsummary$VaRrate_HS,MAsummary$VaR_MA,EWMADanielsson$VaR_rateFixed,NGarchSummary$VaR_rateG,tGarchSummary$VaR_ratetG1,tGarchSummary$dfG,ModelRiskVar)

names(SummaryVar)=c("Stock","HS","MA","EWMA","NGarch","tGarch","df","ModelRisk")
SummaryVar
print(xtable(SummaryVar), include.rownames=FALSE)
...
SummaryES=data.frame(names(ret.cc_rate),HSsummary$ESrate_HS,MAsummary$ES_MA,EWMADanielsson$ES_rateFixed,NGarchSummary$ES_rateG,tGarchSummary$ES_ratetG1,tGarchSummary$dfG,ModelRiskES)

names(SummaryES)=c("Stock","HS","MA","EWMA","NGarch","tGarch","df","ModelRisk")
SummaryES
print(xtable(SummaryES), include.rownames=FALSE)
