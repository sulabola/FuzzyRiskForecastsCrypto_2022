...
```{r}
# download Apple
getSymbols("AAPL", from = '2019-11-25',
...
data<- merge (AAPL$AAPL.Adjusted, Return.calculate(AAPL$AAPL.Adjusted, method = "log"), Return.calculate(AAPL$AAPL.Adjusted, method = "difference"))
...
# step = 63 rolling DDVF volatlity forecasts
ddvf<-c()
...
# step = 63 rolling NNVF volatlity forecasts
nnvf<-c()
...
```{r}
getSymbols("AAPL", from = '2019-08-01',
           to = "2021-11-06",warnings = FALSE,
           auto.assign = TRUE)
nrow(AAPL)
AAPL.Price<-AAPL[, 6]
plot(AAPL.Price, col = "blue")
```


```{r}
temp<-tail(data$LogReturn, 63)

vol.data <-cbind(temp, ddvf.m$DDVF, nnvf.m$NNVF, sd)[, -1]   #temp(tail63)dates
...
```{r}
dygraph(vol.data, main = "63-day rolling Apple volatility forecasts") %>%
...
print(q_numbers[,1]*sqrt(252), digits = 4)
print(q_numbers[,2]*sqrt(252), digits = 4)
```
...
alpha_cuts = function(a,alpha,n){
  return(sqrt(252)*c(a[1]+alpha^{1/0.25}*(a[2]-a[1]), a[4]-alpha^{1/n}*(a[4]-a[3])))
}
```
